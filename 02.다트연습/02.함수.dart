void main(List<String> args) {
  ...
  형식 : (){코드}
  예) 호출 -> 함수명((){코드})
      받음 -> void함수(Function ff) {ff();}

  3. 람다함수 : 함수형변수로 선언 후 화살표로 리턴/실행코드 작성
     형식 : Function 함수명 = (전달변수) => 코드; 
     -> 특징 : return 키워드 없이도 자동 리턴처리됨!
   */
...
showTxt('${leeFight[0]}의 영화 부제목은? ${subTit[leeFight[0]]??'부제목없음'}');
// 이순신 시리즈 영화 부제목 찍기
...
showTxt('다음 이순신 시리즈가 있다면 추천 배우는 공유다!');
showTxt('공유의 취미는 ${recommActor['공유']?['취미']}다. 사는 곳은 ${recommActor['공유']?['사는곳']}다. 공유의 자동차는 ${recommActor['공유']?['자동차']??'정보없음'}다!');
...
// 이순신 시리즈 영화 출연 배우들
// 중괄호 - 'set'Data 중복데이터 없이 출력시킬 때 사용
final actors = {
    '박해일',
    '변요한',
    '최민식',
...
String actorList(List list) {
  var temp = '';
  // for문 : for(시;한;증) {코드}
  // 리스트를 순회하는 for문 : for(변수 in 리스트) {코드}
