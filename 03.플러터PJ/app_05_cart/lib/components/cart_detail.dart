// 카트 페이지 상세 구성 클래스
// 버튼 클릭시 변경되는 변수값에 따라
// 이미지와 정보 표시가 업데이트되는 상태변경 위젯인 
// Stateful 위젯(StatefulWidget)으로 구성한다!!
// stf 단축키로 생성

import 'package:app_o5_cart/constants.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';

// 상태관리 위젯(StatefulWidget) 상속한 메인클래스 ///////////
...
class _CartDetailState extends State<CartDetail> {
  ...
  // 값이 변경되면 이 변수를 사용하는 위젯이 자동업데이트된다!
  ...
  Map<String, List> goodsColor = {
    "Living bicycle": [Colors.amberAccent, Colors.lightBlueAccent, Colors.white],
    "Honda motorcycle": [Colors.red, Colors.purple, Colors.lightBlueAccent],
    "Tesla Model3": [Colors.orange, Colors.deepPurpleAccent, Colors.pink],
    "Cessna 150": [Colors.yellow, Colors.blueGrey],
  };
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        _buildSelector(),
        // 3. 상품정보 위젯 생성 메서드 : _buildCartInfo()
  ...
  Widget _buildPicture() {
    return Padding(
      ...
      child: AspectRatio(aspectRatio: 5/3, child: Image.asset(
        // 위에 셋팅된 이미지를 호출!
  ...
  Widget _buildSelectButton(int seq, IconData mIcon) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        // 현재버튼하고 선택버튼이 같으면 엑센트색 넣기
        color: seq == sequenceNum ? kAccentColor : kSecondaryColor,
        // 둥근모서리 셋팅
        borderRadius: BorderRadius.circular(20),
      ),
        child: IconButton(icon: Icon(mIcon, color: Colors.black,),
        onPressed: () {
          // 여기가 매우매우 중요함
  ...
  // 상품명 + 상품가격 + 별점 + 리뷰수 + 색상옵션 + 버튼 
          ...
          _buildButton(),
              ...
              Widget _buildNamePrice() {
                ...
                child: Row(
                  ...
                  // sequenceNum의 순번값이 변경될 때 이것도 업데이트됨!
                  Text(
                  selectedTit[sequenceNum],
                  style: TextStyle(fontSize:18, fontWeight: FontWeight.bold),
                ),
                  Text(
                    // 달러($)는 특수문자니까 역슬래쉬를 같이씀!
    ...
    return Padding(
      ...
      // 5. Stack의 첫 번째 Container 위젯 위에 
      // Positioned 위젯이 올라가는 형태
      // 스택위젯은 겹쳐지는 디자인을 할 때 사용함!
      // 스택은 아래로부터 위로 겹쳐져서 쌓이는 형태를 이룸!
      // 내부에 겹쳐질 위젯은 Positioned() 위젯을 사용함
      // 이 위젯은 웹에서 absolute 포지션과 유사함(top, left 사용 가능)
      child: Stack(
        children: [
          Container(
            width: 50,
